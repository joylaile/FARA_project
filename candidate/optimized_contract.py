        ...
        for record_key, row in index.items():
            ...
            for field in used_fields:
                ...
                if scalar_key is not None and scalar_key in scalar_fragments:
                    ...
                    encoded = str(value).encode('ascii') if kind is int else _bytes(value)
    ...
    def check(identity, prefix, input_bytes, deps, opcode, left, right):
        ...
        if hit:
            ...
            if cache is not None:
                cache[key] = result
            evaluated += 1
        annotation = dependency_lists.get(deps)
        if annotation is None:
            annotation = list(deps)
            dependency_lists[deps] = annotation
        trace.append({'id':identity, 'passed':result, 'key':key, 'reused':hit, 'dependencies':annotation})
